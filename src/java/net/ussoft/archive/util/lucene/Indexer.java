package net.ussoft.archive.util.lucene;
...
//			Analyzer analyzer = new StandardAnalyzer(Version.LUCENE_36);
	...
	private void createDocument(IndexWriter indexWriter,String tablename,
			List<Sys_templetfield> fieldList,List dataList) throws CorruptIndexException, IOException {
		Document doc = null;
		for (int i=0;i<dataList.size();i++) {
			doc = new Document();
			HashMap tempMap = (HashMap) dataList.get(i);
			for (Sys_templetfield sysTempletfield : fieldList) {
...
//						Analyzer analyzer = new StandardAnalyzer(Version.LUCENE_36);
	...
	private void createDocument(IndexWriter indexWriter,List<Sys_doc> docList,HashMap<String,String> contentMap) throws CorruptIndexException, IOException {
		Document document = null;
		for (int i=0;i<docList.size();i++) {
			document = new Document();
			Sys_doc doc = docList.get(i);
			
			document.add(new Field("id", doc.getId(),Field.Store.YES, Field.Index.NO));
			...
			document.add(new Field("content", contentMap.get(doc.getId()),Field.Store.YES, Field.Index.ANALYZED));
			// 写入IndexWriter
			if (null != document) {
