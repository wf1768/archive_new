...
<script type="text/javascript">
	...
	function pageselectCallback(page_index, jq){
		...
		if (page_index != pageno) {
			window.location.href="${pageContext.request.contextPath}/content/list.do?columnid=${columnid}&page="+page_index+"";
		}
	}; 
//-->
</script>
...
<div class="container" style="margin-top:80px;">
	<div class="row">
		<div class="col-md-12">
		栏目：${column.column_name}  <a href="${pageContext.request.contextPath }/column/list.do" class="btn btn-primary btn-xs">返回栏目列表</a>
			<a href="${pageContext.request.contextPath}/content/insert.do?columnid=${column.id} " class="btn btn-primary btn-xs">添加</a>
			...
			<table class="table table-hover table-condensed table-bordered"><!-- table-striped table-hover table-condensed-->
				...
				<c:forEach items="${pagebean.getList()}" varStatus="i" var="item">
					<tr>
						...
						<td>
							<a href="${pageContext.request.contextPath}/content/edit.do?id=${item.id}" class="btn btn-primary btn-xs">编辑</a>
							<a href="${pageContext.request.contextPath}/content/delete.do?id=${item.id}&columnid=${column.id}&page=${pagebean.pageNo }" onClick="if(confirm('确定要删除信息吗?')==false)return false;" class="btn btn-primary btn-xs">删除</a>
						</td>
					</tr>
				</c:forEach>
			</table>
